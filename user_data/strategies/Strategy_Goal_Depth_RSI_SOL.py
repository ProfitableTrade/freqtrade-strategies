...
class Strategy_Goal_Depth_RSI_SOL(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        rsi = dataframe['rsi'].shift(-13).head(14) < 30
        
        self.logger.info(f"Depth check: {depth_value}, large orders check: {large_orders_value}, volume check: {volume_value.head(1)}, close check: {close_value.head(1)}, rsi: {dataframe['rsi'].head(20)} rsi fixed: {dataframe['rsi'].shift(-13).head(14)}")
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Вихід по RSI
        dataframe.loc[
            (dataframe['rsi'].shift(-13).head(14) > 70) &  
            (dataframe['volume'] > 0),  
            'exit_long'
            ] = 1
